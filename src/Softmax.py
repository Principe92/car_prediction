...
class Softmax(torch.nn.Module):
    ...
    def calc_gradient(self, X_train: np.ndarray, y_train: np.ndarray) -> np.ndarray:
        ...
        for i in range(0,len(X_train)):
                    ...
                    wyx=np.matmul(self.w,np.transpose(X_train[i])) #Calculating initial prediction values
                    #print('wyx')
                    #print(wyx)
                    #print(X_train[i].dot(np.transpose(self.w)))
                    wyx -= np.max(wyx) #making everything non=positive (max of zero) (because e^(non-positive) is between 0 and 1)
                    correct_wyx=wyx[y_train[i]] #getting the prediction value for the correct class
                    #print('Correct_wyx')
                    #print(correct_wyx)
                    ...
                    prob=np.exp(correct_wyx)/sigma #The model's calcualted probability for the correct class is correct for loss function
                    ...
                    for c in range(0,self.n_class):
                        if c==y_train[i]:
                            gradients[y_train[i],:]-= ((np.exp(wyx[y_train[i]])/sigma) -1)*X_train[i] #Adjusting the gradients of the actual class
                            ...
                            gradients[c,:]-= (np.exp(wyx[c])/sigma)*X_train[i] #Adjusting gradients of the incorrect classes
    ...
    def forward(self, X_train: np.ndarray, y_train: np.ndarray):
        ...
        for e in range(0,self.epochs):
            for i in range(0,num_batches):
                ...
                #print(batch_y)
                #print(type(batch_y))
                gradients=Softmax.calc_gradient(self,batch_x,batch_y)
