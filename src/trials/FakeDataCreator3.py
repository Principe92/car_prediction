...
# -*- coding: utf-8 -*-
"""
Created on Sun May  9 20:31:14 2021
...
while stop==0:
    ...
    if len(possible_combos)!=0:
        for i in range(0,len(possible_combos)):
            leftside=data.iloc[possible_combos[i][0]].to_numpy() #Getting values at index (left side of pair)
            rightside=data.iloc[possible_combos[i][1]].to_numpy() #Getting values at index (right side of pair)
            matched_pair=np.concatenate((leftside, rightside), axis=None)
            mixed_data=np.vstack([mixed_data,matched_pair])
            #print(i)
...
mixed_data=pd.DataFrame(mixed_data)
mixed_data['Match']=1
...
num_non_match=5000
non_match_df=np.empty((202,))
while len(non_match_df)<num_non_match:
    index1=np.random.randint(0,len(data2))
    index2=np.random.randint(0,len(data2))
    leftside=data2.iloc[index1].to_numpy() #Getting values at index (left side of pair)
    rightside=data2.iloc[index2].to_numpy() #Getting values at index (right side of pair)
    mismatched_pair=np.concatenate((leftside, rightside), axis=None)
    non_match_df=np.vstack([non_match_df,mismatched_pair])
    print(len(non_match_df))

#%%
#Transposing non_match_df into the right orientation
non_match_df=pd.DataFrame(non_match_df)
